...
3. **Centralize Networking**: In `src/lib/api/`, create a `categories.ts` module with helper functions (`fetchCategories`, `fetchCategoryTree`, `createCategory`, `updateCategory`, `deleteCategory`). Each should wrap `fetch`, parse JSON to the typed shape, and throw meaningful errors.
