...
1. **Create a Feature Directory**: Under `frontend-svelte/src/routes`, add a `categories/` folder with `+page.svelte` and `+page.ts` (if we need server data). Keeping everything in a dedicated directory makes routing explicit.
2. **Define API Types**: Add a `category.ts` file in `frontend-svelte/src/lib/types/` describing the backend DTO (`id`, `name`, `parent_id`, `path`, `usage_count`, etc.). Strong typing helps IntelliSense and reduces runtime surprises.
3. **Centralize Networking**: In `src/lib/api/`, create a `categories.ts` module with helper functions (`fetchCategories`, `createCategory`, `updateCategory`, `deleteCategory`). Each should wrap `fetch`, handle JSON parsing, and throw meaningful errors.
